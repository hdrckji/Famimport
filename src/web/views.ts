...
export function renderDashboard(
  ...
  const cards = [
    [tr.cardImports, stats.totalImports.toString(), tr.cardImportsSub],
    [tr.cardProducts, stats.totalProducts.toLocaleString(lang === "fr" ? "fr-BE" : "nl-BE"), `${eanPct} ${tr.cardProductsSubWithEan}`],
    [tr.cardTarabel, stats.productsWithTarabel.toLocaleString(lang === "fr" ? "fr-BE" : "nl-BE"), `${tarabelPct} ${tr.cardTarabelSub}`],
    [tr.cardUniqueCodes, stats.uniqueTarabelCodes.toString(), tr.cardUniqueCodesSub],
    [tr.cardDeclarations, stats.totalDeclarations.toString(), `${stats.totalCustomsLines} ${tr.cardDeclarationsSub}`],
    [tr.cardUniqueEans, stats.uniqueEans.toString(), `${stats.productsWithEan - stats.uniqueEans} ${tr.cardUniqueEansSub}`],
    [tr.cardDivergence, `${stats.divergencePct}%`, tr.cardDivergenceSub],
  ];
      ...
      <div class="lg:col-span-2 bg-white rounded-lg border border-slate-200">
        ...
        <table class="w-full">
          <thead class="bg-slate-50 text-xs uppercase text-slate-500">
            <tr><th class="py-2 px-3 text-left">${escapeHtml(tr.folder)}</th><th class="py-2 px-3 text-left">${escapeHtml(tr.year)}</th><th class="py-2 px-3 text-left">${escapeHtml(tr.brand)}</th><th class="py-2 px-3 text-right">${escapeHtml(tr.products)}</th><th class="py-2 px-3 text-right">${escapeHtml(tr.validated)}</th></tr>
          </thead>
      ...
      <table class="w-full">
        <thead class="bg-slate-50 text-xs uppercase text-slate-500">
          <tr>
            ...
            <th class="py-2 px-3 text-left">${escapeHtml(tr.tarabelCoverage)}</th>
...
export function renderImportDetail(imp: ImportRow, products: ProductRow[], lang: Lang): string {
  ...
  const rows = products.map((p) => {
    ...
    const sourceLabel = p.tarabel_source === "customs_pdf" ? tr.source_customs : tr.source_packing;
    const tarabelCell = p.tarabel_validated
      ? `<span class="font-mono text-sm">${escapeHtml(p.tarabel_validated)}</span><span class="text-xs text-slate-500 ml-1">(${escapeHtml(sourceLabel)})</span>`
  ...
  const body = `
    <div class="mb-4 flex items-center gap-3">
      ...
      <span class="text-sm text-slate-500">${imp.year} · ${escapeHtml(imp.brand ?? "")} · ${imp.product_count} ${escapeHtml(tr.importDetailHint)} · ${imp.validated_count} ${escapeHtml(tr.validated.toLowerCase())} (${cov}%) · ${escapeHtml(tr.sheet)} "${escapeHtml(imp.schema_variant ?? "")}"</span>
    </div>
...
export function renderProductDetail(p: ProductRow, history: ProductRow[], lang: Lang): string {
  ...
  const sourceLabel = p.tarabel_source === "customs_pdf" ? tr.source_customs : (p.tarabel_source ? tr.source_packing : "");
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-2">
        ...
        <dl class="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
          ...
          <dt class="text-slate-500">${escapeHtml(tr.tarabel)}</dt><dd class="font-mono">${escapeHtml(p.tarabel_validated ?? "—")} ${sourceLabel ? `<span class="text-xs text-slate-500">(${escapeHtml(sourceLabel)})</span>` : ""}</dd>
